// see: https://github.com/travisvn/supabase-pause-prevention?tab=readme-ov-file
...
const fetchOtherEndpoints = async (): Promise<string[]> => {
    if (config?.otherEndpoints != null && config?.otherEndpoints.length > 0) {
        const fetchPromises = config.otherEndpoints.map(async (endpoint) => {
            const endpointResults = await fetch(endpoint, {
                cache: "no-store",
            });
            const passOrFail =
                endpointResults?.status == 200 ? "Passed" : "Failed";
...
export async function GET() {
    ...
    if (config?.disableRandomStringQuery != true) {
        const querySupabaseResponse: QueryResponse =
            await querySupabase(supabase);
